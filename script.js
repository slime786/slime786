...
function clocks(){const n=new Date(),clock=q('#clock'),date=q('#date'),us=q('#us-status');if(clock)clock.textContent=new Intl.DateTimeFormat('en-GB',{timeZone:'Europe/London',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(n);if(date)date.textContent=new Intl.DateTimeFormat('en-GB',{timeZone:'Europe/London',weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(n);if(us){const p=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',weekday:'short',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(n),d=p.find(x=>x.type==='weekday').value,h=+p.find(x=>x.type==='hour').value,m=+p.find(x=>x.type==='minute').value;us.textContent=(!['Sat','Sun'].includes(d)&&h*60+m>=570&&h*60+m<960)?'OPEN':'CLOSED'}}clocks();setInterval(clocks,1000);
...
async function crypto(){try{const d=await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum&vs_currencies=gbp&include_24hr_change=true',{cache:'no-store'}).then(r=>r.json());const pairs=[['btc',gbp(d.bitcoin.gbp)],['btc2',gbp(d.bitcoin.gbp)],['eth',gbp(d.ethereum.gbp)],['eth2',gbp(d.ethereum.gbp)],['btc-change',chg(d.bitcoin.gbp_24h_change)],['eth-change',chg(d.ethereum.gbp_24h_change)]];pairs.forEach(([id,v])=>{const e=q('#'+id);if(e)e.textContent=v})}catch{const b=q('#btc'),e=q('#eth');if(b)b.textContent='Unavailable';if(e)e.textContent='Unavailable'}}crypto();setInterval(crypto,60000);
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
async function news(){const f=q('#news');if(!f)return;try{const ids=await fetch('https://hacker-news.firebaseio.com/v0/topstories.json').then(r=>r.json()),ss=await Promise.all(ids.slice(0,5).map(id=>fetch(`https://hacker-news.firebaseio.com/v0/item/${id}.json`).then(r=>r.json())));f.innerHTML=ss.filter(Boolean).map(s=>`<div class='news-item'><a href='${esc(s.url||`https://news.ycombinator.com/item?id=${s.id}`)}' target='_blank' rel='noopener'>${esc(s.title)}</a><small>${s.score||0} points</small></div>`).join('')}catch{f.textContent='Technology feed temporarily unavailable.'}}news();setInterval(news,300000);
